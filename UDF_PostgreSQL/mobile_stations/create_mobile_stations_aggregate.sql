CREATE FUNCTION tb_gis_mobile_stations_aggregate(table_name text, target text, p1_x float, p1_y float, p2_x float, p2_y float, p3_x float, p3_y float, p4_x float, p4_y float, st timestamp, et timestamp, gr text, proc text)
...
query  = "SELECT *, ST_X(location) AS longitude, ST_Y(location) AS latitude from " + table_name + " "
...
for row in plpy.cursor(query):
    mobile_location.append([str(row['IDENTIFIER']), row['datetime'], float(row['longitude']), float(row['latitude']), row['COURSE'], row['SPEED']])
    mobile_data.append([str(row['IDENTIFIER']), row['datetime'], row[target]])

if len(mobile_location) == 0:
    return []

df_location = pandas.DataFrame(mobile_location, columns=['id', 'datetime', 'lng', 'lat', 'direction', 'speed'])
...
for id_name, group in df_location.groupby('id'):
    ...
    df_location_dict[id_name] = work.resample(gr).first()
    df_location_dict[id_name]['id'] = id_name
    df_location_dict[id_name] = df_location_dict[id_name].reset_index()

df_data = pandas.DataFrame(mobile_data, columns=['id', 'datetime', 'data'])
...
for id_name, group in df_data.groupby('id'):
    ...
    work.set_index('datetime', inplace=True)
    if proc == 'max':
        df_data_dict[id_name] = work.resample(gr).max()
    elif proc == 'min':
        df_data_dict[id_name] = work.resample(gr).min()
    else :
        df_data_dict[id_name] = work.resample(gr).mean()
