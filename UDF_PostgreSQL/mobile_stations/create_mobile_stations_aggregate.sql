CREATE OR REPLACE FUNCTION tb_gis_mobile_stations_aggregate(
    table_name text,
    id_column text,
...
for id_name, group in df_mobile.groupby('id'):
    ...
    work_location = work[['longitude','latitude']].resample(freq).first()
    work_data = []
    if method == 'max':
        work_data = work[data_key_list].resample(freq).max()
    elif method == 'min':
        work_data = work[data_key_list].resample(freq).min()
    else :
        work_data = work[data_key_list].resample(freq).mean()

    df_dict[id_name] = pandas.merge(work_location, work_data, on=['datetime'])
...
df_result = df_result.reindex(columns=['id', 'datetime', 'longitude', 'latitude', 'datalist'])
